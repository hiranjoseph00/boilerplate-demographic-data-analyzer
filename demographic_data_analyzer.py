...
def calculate_demographic_data(print_data=True):
    ...
    lower_education = len(led).isin(['Bachelors','Masters','Doctorate'])]
    ...
    total_rich_countries=df[df['salary']=='>50K']['native-country'].value_counts()
    
    highest_earning_country = (total_rich_countries/total_countries*100).idxmax()
    highest_earning_country_percentage = (total_rich_countries / total_countries *100).max()

    # Identify the most popular occupation for those who earn >50K in India.
    poi=df[df['native-country']=='India']
    total_occupations=poi['occupation'].value_counts()
    top_IN_occupation = total_occupations.idxmax()
